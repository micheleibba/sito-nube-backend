...
@section('title', 'Modifica Q&A - Nube')
...
<div class="flex items-center gap-2 text-sm mb-6">
    ...
    <span class="text-surface-400">{{ $qa->section->name }}</span>
    <svg class="w-3.5 h-3.5 text-surface-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
    <span class="text-surface-300">Modifica</span>
</div>

<div class="max-w-2xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold tracking-tight">Modifica Q&A</h1>
    </div>

    <div class="bg-surface-900/50 border border-surface-800/50 rounded-2xl p-6">
        <form method="POST" action="{{ route('chat-qa.update-qa', $qa) }}" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="question" class="block text-xs font-medium text-surface-400 mb-1.5">Domanda (in inglese)</label>
                <input type="text" id="question" name="question" value="{{ old('question', $qa->question) }}" required
            ...
            <div>
                ...
                <textarea id="answer" name="answer" rows="6" required
                    class="input-field w-full px-3.5 py-2.5 bg-surface-950/50 border border-surface-700/50 rounded-xl text-sm text-white placeholder-surface-600 focus:outline-none resize-y">{{ old('answer', $qa->answer) }}</textarea>
                @error('answer')
                    <p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>
                @enderror
            </div>
